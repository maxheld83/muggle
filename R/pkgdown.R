...
#' These replacement versions of pkgdown functions make the following changes
#'  to pkgdown, as applicable:
...
#'    by appending [override_pkgdownyaml()]) to `override`.
#'    Be careful not to provide conflicting overrides.
#' - Sets `run_dont_run = TRUE`, so that examples inside `\dontrun{}`
...
#'
#' @details
#' This `vignettes/_site.yml` declares the default vignette rendering function
...
#' sections supported by [bookdown](http://bookdown.org).
#' The
...
#'
#' ## Raw HTML
#' Pandoc extensions are set to allow correctly indented raw HTML
#' inside vignettes.
#'
...
#' That's assumed to be the public-facing website,
#' such as a pkgdown website on GitHub pages.
#'
#' @keywords internal
