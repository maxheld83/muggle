...
#' This is the inverse of the the `azure login` GitHub Action,
...
#'    and the corresponding **static GitHub username**.
...
#' 2. You can authenticate Azure towards GitHub Packages
#'    **with the `GITHUB_ACTOR` and `GITHUB_TOKEN` pair**
#'    [furnished](https://docs.github.com/en/actions/configuring-and-managing-workflows/authenticating-with-the-github_token)
#'    for GitHub Actions, from which the deployment takes place.
#'    The `GITHUB_ACTOR` environment variable will always be
#'    the GitHub username of whoever triggered the workflow run,
#'    and therefore, the deployment.
#'    The `GITHUB_TOKEN` is a PAT scoped to *only* that repo and will expire
#'    automatically after the workflow run is completed.
#'    To use this authentication, use the below defaults and expose the
#'    `GITHUB_TOKEN` to in your workflow `*.yaml` file.
#'
#'    ```yaml
#'    env:
...
#'    though the `GITHUB_TOKEN` still has more scope than just to read packages,
...
#'    it will not be able to `docker pull` the image again,
#'    because the `GITHUB_TOKEN` will have expired by then.
...
az_webapp_config_muggle <- function(name = Sys.getenv("MUGGLE_PKG_NAME"),
                                    ...
                                    resource_group,
                                    subscription,
                                    docker_registry_server_user = Sys.getenv(
                                      "GITHUB_ACTOR"
                                    ),
                                    docker_registry_server_password = Sys.getenv(
                                      "GITHUB_TOKEN"
                                    ),
                                    restart = FALSE) {
